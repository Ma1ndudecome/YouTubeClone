     ...
     const IDVideo =  response.data.items.map(el=> el.id.videoId).join(',')
      ...
      .then(data=>{
        data.data.items.forEach(el=>{
          ...
          container.insertAdjacentHTML("afterbegin", makeMarkingVideo(el.snippet.thumbnails.high.url, el.snippet.thumbnails.default.url, el.snippet.title, el.snippet.channelTitle,el.statistics.viewCount, result )) 
        })
      })
...
const dateString = '2023-01-24T12:15:02Z';
